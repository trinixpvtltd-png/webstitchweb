...
export default function Blog() {
  ...
  const allCategories = ['All', ...new Set(articles.map(article => article.tag))];
  const allTags = ['All', ...new Set(articles.flatMap(article => [article.tag]))];
            ...
            {!loading && !error && (
              <>
                ...
                {featuredArticles.length > 0 ? (
                      ...
                      <Reveal key={article._id} delay={index * 100}>
                        <Card className="bg-white rounded-2xl border border-[#e3e8f0] shadow-xl hover:shadow-2xl transition-all overflow-hidden h-full flex flex-col dark:bg-[#23263a] dark:border-[#353a50] dark:shadow-2xl">
                          <div className="relative h-60 overflow-hidden">
                            ...
                            <img 
                              src={article.image || `https://placehold.co/800x400/ede9fe/7c3aed?text=${article.tag}`} 
                              alt={article.heading} 
                              className="w-full h-full object-cover transition-transform hover:scale-105"
                            />
                          </div>
                          <CardContent className="flex-grow pt-6">
                            <h3 className="text-xl font-semibold mb-2 line-clamp-2 text-[#1a237e] dark:text-white">{article.heading}</h3>
                            <p className="mb-4 line-clamp-3 text-[#3f51b5] dark:text-[#a5b4fc]">{article.description}</p>
                            ...
                            <div className="flex items-center justify-between w-full">
                              <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-[#ede9fe] overflow-hidden">
                                  <img 
                                    src={`https://placehold.co/100/ede9fe/7c3aed?text=${article.author.charAt(0)}`} 
                                    alt={article.author}
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-[#5c6bc0] dark:text-[#a5b4fc]">{article.authorPosition}</p>
                                </div>
                              </div>
                  ...
                  <div className="text-center py-8 text-[#3f51b5] dark:text-[#a5b4fc]">No featured articles yet.</div>
                )}
              </>
            )}
          </div>
            ...
            {!loading && !error && (
              <>
                ...
                {filteredArticles.length > 0 ? (
                      ...
                      <Reveal key={article._id} delay={index * 75}>
                        <Card className="bg-white/10 backdrop-blur-md rounded-2xl border border-[#e3e8f0]/30 shadow hover:shadow-xl transition-all overflow-hidden h-full flex flex-col dark:bg-[#23263a]/30 dark:border-[#353a50]/30 dark:shadow-2xl">
                          <div className="relative h-48 overflow-hidden">
                            ...
                            <img 
                              src={article.image || `https://placehold.co/600x300/ede9fe/7c3aed?text=${article.tag}`} 
                              alt={article.heading} 
                              className="w-full h-full object-cover transition-transform hover:scale-105"
                            />
                          </div>
                          <CardContent className="flex-grow pt-5">
                            ...
                            <h3 className="text-lg font-semibold mb-2 line-clamp-2 text-[#1a237e] dark:text-white">{article.heading}</h3>
                            <p className="text-sm mb-3 line-clamp-2 text-[#3f51b5] dark:text-[#a5b4fc]">{article.description}</p>
                  ...
                  <div className="text-center py-8 text-[#3f51b5] dark:text-[#a5b4fc]">No articles found for this category.</div>
                )}
