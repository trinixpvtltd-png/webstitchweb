          ...
          {projectsLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {featuredProjects.map((project, i) => (
                <div key={project._id || i} className="bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col border border-[#e3e8f0] dark:bg-[#23263a] dark:border-[#353a50] dark:shadow-2xl">
                  <div className="relative h-56 w-full overflow-hidden">
                    <img src={project.image} alt={project.title} className="w-full h-full object-cover" />
                    <div className="absolute top-4 left-4 flex gap-2">
                      {project.tags && project.tags.map((tag: string, j: number) => (
                        <Badge key={j} className={`px-3 py-1 text-xs rounded-full font-bold shadow bg-[#7c3aed] text-white dark:bg-[#a78bfa] dark:text-white`}>{tag}</Badge>
                      ))}
                      {project.isFeatured && (
                        <Badge className="px-3 py-1 text-xs rounded-full font-bold shadow bg-[#c7d2fe] text-[#1a237e] dark:bg-[#6366f1] dark:text-white">Featured</Badge>
                      )}
                    </div>
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-4 mb-4">
                      {project.stats && project.stats.map((stat: any, k: number) => (
                    ...
                    <div className="mb-4">
                      <div className="text-sm font-semibold text-[#3f51b5] mb-2 dark:text-[#a5b4fc]">Tech Stack:</div>
                      <div className="flex flex-wrap gap-2">
                        {project.techStack && project.techStack.map((tech: string, l: number) => (
                          <Badge key={l} className="bg-light-purple-100 text-purple-800 px-3 py-1 text-xs rounded-full border border-purple-200 font-semibold dark:bg-purple-900 dark:text-purple-200 dark:border-purple-700">{tech}</Badge>
                        ))}
                      </div>
                    </div>
                  </div>
          ...
          {projectsLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {allProjects.map((project, i) => (
                <div key={project._id || i} className="bg-white rounded-2xl overflow-hidden shadow-xl flex flex-col border border-[#e3e8f0] dark:bg-[#23263a] dark:border-[#353a50] dark:shadow-2xl">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="mb-3">
                      <div className="text-xs font-semibold text-[#3f51b5] mb-2 dark:text-[#a5b4fc]">Tech Stack:</div>
                      <div className="flex flex-wrap gap-2">
                        {project.techStack && project.techStack.map((tech: string, l: number) => (
                          <Badge key={l} className="bg-purple-100 text-purple-800 px-3 py-1 text-xs rounded-full border border-purple-200 font-semibold dark:bg-purple-900 dark:text-purple-200 dark:border-purple-700">{tech}</Badge>
                        ))}
                      </div>
                    </div>
                  </div>
