...
import { getServices, addService, editService, deleteService, getProjects, addProject, editProject, deleteProject, getArticles, addArticle, editArticle, deleteArticle } from '../lib/Api';
...
const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'services' | 'projects' | 'articles'>('services');
  // Article state
        ...
        <div className="flex pt-20 gap-8">
          <aside className="w-72 bg-gradient-to-br from-[#23263a]/80 to-[#6366f1]/30 backdrop-blur-xl shadow-2xl p-8 flex flex-col rounded-3xl border-r border-white/10 min-h-[80vh]">
            ...
            <nav className="flex flex-col gap-4">
              ...
              <button
                className={`text-left px-5 py-3 rounded-xl transition font-semibold text-lg ${activeTab === 'articles' ? 'bg-gradient-to-r from-[#7c3aed] to-[#6366f1] text-white shadow-xl' : 'text-[#a5b4fc] hover:bg-white/10'}`}
          ...
          <main className="flex-1 p-8">
            ...
            {activeTab === 'articles' && (
              <div>
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-3xl font-extrabold bg-gradient-to-r from-[#7c3aed] via-[#6366f1] to-[#a5b4fc] bg-clip-text text-transparent drop-shadow-lg">Articles</h2>
                  <button
                    className="bg-gradient-to-r from-[#7c3aed] to-[#6366f1] text-white px-6 py-3 rounded-xl font-bold shadow-xl hover:from-[#7c3aed]/80 hover:to-[#6366f1]/80 transition text-lg"
                    ...
                    <table className="w-full border-separate border-spacing-y-2">
                      ...
                      <tbody>
                        {articles.map((article: any) => (
                          <tr key={article._id} className="rounded-xl bg-black/60 hover:bg-white/10 transition shadow-xl">
                            <td className="p-4 rounded-l-xl font-semibold text-white/90 text-center">{article.heading}</td>
                            ...
                            <td className="p-4 flex gap-2 rounded-r-xl justify-center">
                              ...
                              >Edit</button>
                              <button className="px-4 py-2 bg-red-500/80 text-white rounded-lg hover:bg-red-600 transition font-semibold" onClick={() => handleDeleteArticle(article._id)}>Delete</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                {showArticleModal && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    <div className="bg-black rounded-2xl shadow-2xl p-8 w-full max-w-lg relative border border-white/10">
                      <button className="absolute top-3 right-3 text-gray-400 hover:text-white text-2xl" onClick={() => setShowArticleModal(false)}>&times;</button>
                      <h3 className="text-xl font-bold mb-4 text-center text-[#7c3aed]">Create Article</h3>
                      <form onSubmit={e => { e.preventDefault(); handleAddArticle(articleForm); }} className="flex flex-col gap-4">
                        <input className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Heading" required value={articleForm.heading} onChange={e => setArticleForm(f => ({ ...f, heading: e.target.value }))} />
                        <textarea className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Description" required value={articleForm.description} onChange={e => setArticleForm(f => ({ ...f, description: e.target.value }))} />
                        <input className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Tag" required value={articleForm.tag} onChange={e => setArticleForm(f => ({ ...f, tag: e.target.value }))} />
                        <input className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Image URL (optional)" value={articleForm.image} onChange={e => setArticleForm(f => ({ ...f, image: e.target.value }))} />
                ...
                {showEditArticleModal && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    <div className="bg-black rounded-2xl shadow-2xl p-8 w-full max-w-lg relative border border-white/10">
                      <button className="absolute top-3 right-3 text-gray-400 hover:text-white text-2xl" onClick={() => setShowEditArticleModal(false)}>&times;</button>
                      <h3 className="text-xl font-bold mb-4 text-center text-[#7c3aed]">Edit Article</h3>
                      <form onSubmit={e => { e.preventDefault(); if (editArticleId) handleEditArticle(editArticleId, editArticleForm); }} className="flex flex-col gap-4">
                        <input className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Heading" required value={editArticleForm.heading} onChange={e => setEditArticleForm(f => ({ ...f, heading: e.target.value }))} />
                        <textarea className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Description" required value={editArticleForm.description} onChange={e => setEditArticleForm(f => ({ ...f, description: e.target.value }))} />
                        <input className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Tag" required value={editArticleForm.tag} onChange={e => setEditArticleForm(f => ({ ...f, tag: e.target.value }))} />
                        <input className="border border-white/20 rounded-lg px-4 py-2 bg-black text-white placeholder-gray-400" placeholder="Image URL (optional)" value={editArticleForm.image} onChange={e => setEditArticleForm(f => ({ ...f, image: e.target.value }))} />
